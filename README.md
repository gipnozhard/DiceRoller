...
## 5. Использование журналов для выявления и исправления ошибки

В предыдущем разделе вы рассмотрели трассировку стека, в частности, эту строку.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/dda5ac6d-b08a-4688-91a8-56977014a85d)

Здесь вы можете сказать, что сбой произошел в строке 21 и был связан с делением на ноль. Таким образом, где-то перед выполнением этого кода знаменатель был равен 0. Хотя вы могли бы попробовать пошагово выполнить код самостоятельно, что могло бы отлично сработать для небольшого примера, подобного этому, вы также можете использовать инструкции log для экономии времени, печатая значение знаменателя до того, как произойдет деление на ноль.

 1. Перед операцией Log.v(), добавьте Log.d() вызов, который регистрирует знаменатель. Log.d() используется потому, что этот журнал предназначен специально для отладки, и чтобы вы могли отфильтровывать подробные журналы.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/b2fc9acb-040d-4c2b-9634-d374a9c45bab)

 2. Запустите ваше приложение еще раз. Хотя оно по-прежнему вылетает, знаменатель следует записать в журнал несколько раз. Вы можете использовать конфигурацию фильтра, чтобы показывать журналы только с "MainActivity" тегом.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/e8351d5e-e645-4434-b4c6-78e6d8b391f9)

 3. Вы можете видеть, что напечатано несколько значений. Похоже, что цикл выполняется несколько раз, прежде чем завершиться сбоем на пятой итерации, когда знаменатель равен 0. Это имеет смысл, потому что знаменатель равен 4, и цикл уменьшает его на 1 для 5 итераций. Чтобы исправить ошибку, вы можете изменить количество итераций в цикле с 5 на 4. Если вы повторно запустите приложение, оно больше не должно аварийно завершать работу.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/806bfd60-bde3-4df1-8944-bac08b27fd24)

## 6. Пример отладки: доступ к значению, которого не существует

По умолчанию Пустой шаблон Activity, который вы использовали для создания проекта, добавляет одно действие с TextView центром на экране. Как вы узнали ранее, вы можете ссылаться на представления из кода, установив идентификатор в редакторе макетов и получив доступ к представлению с помощью findViewById(). Когда onCreate() вызывается класс activity, сначала необходимо вызвать его, setContentView() чтобы загрузить файл макета (например, activity_main.xml). Если вы попытаетесь вызвать, findViewById() перед вызовом setContentView(), приложение завершит работу, потому что представление не существует. Давайте попробуем получить доступ к представлению, чтобы проиллюстрировать другую ошибку.

 1. Откройте activity_main.xml, выберите Привет, мир! TextView и установите id значение hello_world.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/100d72b7-45bb-47b1-9052-88c3e82165c9)

 2. Вернитесь в ActivityMain.kt in onCreate(), добавьте код для получения TextView и измените его текст на "Привет, отладка!" перед вызовом setContentView().

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/84f75d9b-9a27-435b-a03b-e677f93eed4e)

 3. Запустите приложение еще раз и обратите внимание, что оно снова вылетает сразу после запуска. Возможно, вам потребуется удалить фильтр из предыдущего примера, чтобы увидеть журналы без "MainActivity" тега.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/6f0be723-d624-4c5c-bf0a-4c3e114ce170)

Исключение должно появляться в Logcat одним из последних (если нет, вы можете выполнить поиск RuntimeException). Результат должен выглядеть следующим образом.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/0306cb48-cccf-4e75-ab5d-79adb5fa6746)

Как и раньше, вверху написано "Не удается запустить activity". Это имеет смысл, поскольку приложение завершило работу еще до MainActivity запуска. В следующей строке немного подробнее рассказывается об ошибке.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/6ac8536a-ae2b-4c1b-b9a5-9c6fa6574600)

Далее в трассировке стека вы также увидите эту строку, показывающую точный вызов функции и номер строки.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/a65351c4-999e-4bb8-925b-1f53415d05e3)

Что именно означает эта ошибка и что именно представляет собой "нулевое" значение? Хотя это надуманный пример и у вас, возможно, уже есть представление о причинах сбоя приложения, вы неизбежно столкнетесь с сообщениями об ошибках, которых раньше не видели. Когда это происходит, скорее всего, вы не первый, кто видит ошибку, и даже самые опытные разработчики часто просматривают сообщение об ошибке в Google, чтобы узнать, как другие решили проблему. Поиск этой ошибки приводит к нескольким результатам на StackOverflow, сайте, где разработчики могут задавать вопросы и давать ответы о багги-коде или более общих темах программирования.

Поскольку может быть много вопросов с похожими, но не совсем одинаковыми ответами, помните о следующих советах при самостоятельном поиске ответов.

 1. Сколько лет ответу? Ответы, полученные несколько лет назад, могут больше не быть актуальными или могут использовать устаревшую версию языка или фреймворка.

 2. Для ответа используется Java или Kotlin? Ваша проблема специфична для того или иного языка или связана с конкретной платформой?

 3. Ответы, отмеченные как "принятые" или с большим количеством положительных отзывов, могут быть более высокого качества, но имейте в виду, что другие ответы все еще могут предоставить ценную информацию.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/5f88ef3b-a28e-4de7-816d-c0b2c7f242e1)

Число указывает на количество положительных (или отрицательных) голосов, а зеленая галочка указывает на принятый ответ.

Если вы не можете найти существующий вопрос, вы всегда можете задать новый. Задавая вопрос на StackOverflow (или любом другом сайте), рекомендуется помнить об этих рекомендациях.

Продолжайте искать ошибку.


![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/92a7b257-71a9-41ec-bbf4-bcc508a35d86)

Если вы прочтете некоторые ответы, то обнаружите, что ошибка может иметь несколько различных причин. Однако, учитывая, что вы намеренно вызывали findViewById() раньше setContentView(), некоторые ответы на этот вопрос звучат многообещающе. Второй по количеству проголосовавших ответ, например, гласит:
...
После просмотра этого ответа вы можете затем проверить в коде, что действительно вызов findViewById() слишком ранний, происходит до setContentView(), и что вместо этого его следует вызывать после setContentView().

Исправьте ошибку, обновив код.

 1. Переместите вызов в findViewById() и строку, которая задает helloTextViewтекст под вызовом в setContentView(). Новый onCreate() метод должен выглядеть так, как показано ниже. Вы также можете добавлять журналы, как показано на рисунке, чтобы убедиться, что ошибка исправлена.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/70ca8535-2a56-4848-b1f7-ffbc21b90273)

Запустите приложение повторно. Обратите внимание, что приложение больше не выходит из строя, а текст обновляется, как ожидалось.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/b4c796e9-0898-4e7b-94c0-93eaa94870b3)

### Делайте скриншоты

К настоящему моменту вы, вероятно, видели множество скриншотов из эмулятора Android в этом курсе. Создание скриншотов относительно просто, но может быть полезно для обмена информацией, такой как шаги по воспроизведению ошибок, с другими членами команды. Вы можете сделать снимок экрана в эмуляторе Android, нажав на значок камеры на панели инструментов справа.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/0ed93de3-e7fe-41ac-a642-1855470e8849)

Вы также можете использовать сочетание клавиш Command + S, чтобы сделать снимок экрана. Снимок экрана автоматически сохраняется в папке вашего рабочего стола.

### Запишите запущенное приложение

Хотя скриншоты могут содержать много информации, иногда полезно поделиться записями запущенного приложения, чтобы помочь другим воспроизвести то, что вызвало ошибку. Эмулятор Android предлагает несколько встроенных инструментов, которые помогут вам легко создавать GIF (анимированное изображение) запущенного приложения.

 1. В инструментах эмулятора справа нажмите кнопку Еще 558dbea4f70514a8.png (последняя опция), чтобы отобразить дополнительные параметры отладки эмулятора. Появится окно, предоставляющее дополнительные инструменты для моделирования функциональности физических устройств в целях тестирования.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/8868d1c2-0d7e-4b56-9365-d3a5fa18eb93)

 2. В меню слева выберите "Запись и воспроизведение", и вы увидите экран с кнопкой для начала записи.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/092dd369-a2ed-4a83-a5c4-f5559746b2c6)

 3. Прямо сейчас в вашем проекте нет ничего интересного для записи, кроме статики TextView. Давайте изменим код, чтобы обновлять метку каждые несколько секунд, чтобы показать результат деления. В division() методе in MainActivity добавьте вызов Thread.sleep(3000) перед вызовом to Log(). Теперь метод должен выглядеть следующим образом (обратите внимание, что цикл должен повторяться только 4 раза, чтобы избежать сбоя).

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/d99e7184-55e6-4ac3-bd59-9904900d4663)

 4. В activity_main.xmlустановите для id TextViewзначения division_textview.


![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/b9b86f24-bc42-49c8-929e-3a81dd5facc1)

 5. Вернитесь в MainActivity.kt, замените вызов Log.v() на следующие вызовы findViewById() и setText(), чтобы присвоить тексту значение частного.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/d987ba1e-39c7-44db-b92f-e3bd04e7fb27)

 6. Поскольку сейчас вы визуализируете результат разделения в пользовательском интерфейсе приложения, вам необходимо позаботиться о некоторых деталях запуска обновлений пользовательского интерфейса. Во-первых, вам нужно создать новый поток, который может запускать repeat цикл. В противном случае Thread.sleep(3000) был бы заблокирован основной поток, и представление приложения не отображалось бы до onCreate() завершения (включая division() его repeat цикл).

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/558cc843-4bd0-47ac-a357-212d0270764d)

 7. Если вы попытаетесь запустить приложение сейчас, вы заметите FATAL EXCEPTION. Причина этого исключения в том, что только потокам, создавшим представление, разрешено изменять его. К счастью, вы можете ссылаться на поток пользовательского интерфейса, используя runOnUiThread(). Изменитеdivision(), чтобы обновить TextView в потоке пользовательского интерфейса.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/356b23ed-4be3-493e-bed5-cf5fd9712832)

Запустите свое приложение и затем немедленно переключитесь на эмулятор. Когда приложение запустится, нажмите кнопку Начать запись в окне расширенного управления. Вы должны увидеть, что коэффициент обновляется каждые три секунды. После того, как коэффициент будет обновлен несколько раз, нажмите Остановить запись.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/07db0307-b802-472f-861a-466a8f1b04a8)

 9. По умолчанию выходные данные сохраняются в .webm формате. Используйте выпадающий список, чтобы экспортировать выходные данные в виде GIF-файла.

![image](https://github.com/gipnozhard/DiceRoller/assets/71705375/6fc453b6-400d-4ac9-bb11-92a8e780f93e)

## 7. Поздравляем

Поздравляем! На этом пути вы узнали, что:

 * Отладка - это процесс устранения ошибок в вашем коде.

 * Журнал позволяет печатать текст с различными уровнями журнала и тегами.

 * Трассировка стека предоставляет информацию об исключении, такую как точная функция, вызвавшая его, и номер строки, в которой возникло исключение.

 * При отладке, скорее всего, кто-то столкнулся с такой же или похожей проблемой, и вы можете использовать сайты вроде StackOverflow для поиска ошибки.

 * Вы можете легко экспортировать как скриншоты, так и анимированные GIF-файлы с помощью эмулятора Android.
